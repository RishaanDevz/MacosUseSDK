...
/// - Throws: `MacosUseSDKError` if the event source cannot be created or the event cannot be posted.
public func scrollWheel(at point: CGPoint, deltaY: Int32 = 0, deltaX: Int32 = 0) throws {
    fputs("log: simulating scroll at: (\(point.x), \(point.y)), deltaY: \(deltaY), deltaX: \(deltaX)\n", stderr)
    let source = try createEventSource()

    // Create scroll wheel event
    let scrollEvent = CGEvent(scrollWheelEvent2Source: source,
                              units: .line, // Specify scroll units (lines)
                              wheelCount: 2, // 2 dimensions (horizontal and vertical)
                              wheel1: deltaY,
                              wheel2: deltaX,
                              wheel3: 0)
    ...
    try postEvent(scrollEvent, actionDescription: "scroll at (\(point.x), \(point.y))")
    fputs("log: scroll simulation complete.\n", stderr)
}

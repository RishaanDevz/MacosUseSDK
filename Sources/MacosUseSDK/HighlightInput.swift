...
///   - deltaY: Vertical scroll amount. Positive values scroll down, negative values scroll up.
///   - deltaX: Horizontal scroll amount. Positive values scroll right, negative values scroll left.
///   - duration: How long the visual feedback should last (in seconds). Default is 0.5s.
/// - Throws: `MacosUseSDKError` if simulation or visualization fails.
public func scrollWheelAndVisualize(at point: CGPoint, deltaY: Int32 = 0, deltaX: Int32 = 0, duration: Double = 0.5) throws {
    fputs("log: simulating scroll AND visualize at: (\(point.x), \(point.y)), deltaY: \(deltaY), deltaX: \(deltaX), duration: \(duration)s\n", stderr)

    // Call the scroll function
    try scrollWheel(at: point, deltaY: deltaY, deltaX: deltaX)

    // Create appropriate caption text based on scroll direction
    var captionText = "[SCROLL "
    if deltaY > 0 {
        captionText += "DOWN"
    } else if deltaY < 0 {
        captionText += "UP"
    ...
    DispatchQueue.main.async {
        Task { @MainActor in
            // Show a circle at the scroll point
            showVisualFeedback(at: point, type: FeedbackType.circle, duration: duration)

            // Show caption at screen center
            if let screenCenter = getMainScreenCenter() {
                showVisualFeedback(
                    at: screenCenter,
                    type: .caption(text: captionText),
    ...
    fputs("log: scroll simulation and visualization dispatched.\n", stderr)
}

...
actions:
  ...
  scroll <x> <y> <deltaY> <deltaX> Simulate a mouse scroll event at screen coordinates.
...
Examples:
  ...
  \(scriptName) scroll 500 500 -10 0  # Scroll up slightly at (500, 500)
"""
...
do {
    ...
    case "scroll":
        guard arguments.count == 6 else {
            throw MacosUseSDKError.inputInvalidArgument("'scroll' requires exactly four arguments: <x> <y> <deltaY> <deltaX>\n\(usage)")
        }
        guard let x = Double(arguments[2]), let y = Double(arguments[3]) else {
            throw MacosUseSDKError.inputInvalidArgument("Invalid coordinates for 'scroll'. x and y must be numbers.")
        }
        guard let deltaY = Int32(arguments[4]), let deltaX = Int32(arguments[5]) else {
            throw MacosUseSDKError.inputInvalidArgument("Invalid delta values for 'scroll'. deltaY and deltaX must be integers.")
        }
        let point = CGPoint(x: x, y: y)
        log("Coordinates: (\(x), \(y)), DeltaY: \(deltaY), DeltaX: \(deltaX)")
